...
bool write_json_stream(const std::string& type, const std::string& axiom, const std::string& description, const std::string& rate) {
	if (!enable_streaming)
		return true;
...
template<typename Stream>
bool read_terms(array<hol_term*>& terms, Stream& in, hash_map<string, unsigned int>& names) {
...
inline bool observations_has_test_proof(
    const Theory& T,
    const log_probability_collector<ProofCalculus, Canonicalizer>& collector) {
	return T.observations.contains(collector.test_proof);
}

template<typename Theory, typename ProofAxioms, typename Collector>
bool check_consistency(Theory& T, const ProofAxioms& proof_axioms, const Collector& collector, unsigned int i, unsigned int j, unsigned int t, const char* phase) {
...
int main(int argc, const char** argv) {
	...
	theory<natural_deduction<hol_term, false>,
	       polymorphic_canonicalizer<true, false, built_in_predicates>>
	    T(seed_axioms, 1000000000);
	...
	auto constant_prior = make_simple_constant_distribution(
	    iid_uniform_distribution<unsigned int>(10000),
	    chinese_restaurant_process<unsigned int>(1.0, 0.0),
	    make_dirichlet_process(1.0e-1,
	                           make_dirichlet_process(1000.0,
	                                                  make_iid_uniform_distribution<hol_term>(10000))));

	auto theory_element_prior = make_simple_hol_term_distribution<built_in_predicates>(
	    constant_prior, geometric_distribution(0.0001), very_light_tail_distribution(-40.0), 0.0199999, 0.01, 0.0000001, 0.17, 0.1, 0.1, 0.01, 0.57, 0.01, 0.01, 0.1099999, 0.01, 0.0000001, 0.1999999, 0.26, 0.01, 0.01, 0.0000001, 0.2, 0.2, 0.999999998, 0.000000001, 0.000000001, 0.3, 0.4, 0.2, 0.4, -2000.0);
	...
	auto conjunction_elimination_prior = make_levy_process(poisson_distribution(2.0),
	...
	auto term_indices_prior = make_levy_process(poisson_distribution(4.0),
	                                            poisson_distribution(1.5));
	auto proof_prior = make_canonicalized_proof_prior(
	    axiom_prior, conjunction_introduction_prior, conjunction_elimination_prior, universal_introduction_prior, universal_elimination_prior, term_indices_prior, poisson_distribution(20.0), 0.00001);
	...
	for (unsigned int i = 0; i + 1 < lfs.length; i++) {
		...
		if (enable_streaming) {
			...
			write_json_stream("axiom", to_string(axiom_str), "Added logical form to theory", to_string(rate_str));
		}
		...
		fflush(stdout);
		print("Theory log probability: ", stdout);
		print(max_log_probability, stdout);
		print("\n\n", stdout);
		...
		if (enable_streaming) {
			...
			write_json_stream("axiom", to_string(axiom_str), "Best theory after optimization", to_string(rate_str));
		}
	...
	if (lfs.last()->type != hol_term_type::LAMBDA) {
		...
		Proof* proof_MAP;
		double log_probability_true = log_joint_probability_of_truth(
		    T, proof_prior, proof_axioms, lfs.last(), 250, 4, 20, T_MAP, proof_MAP);
		...
		double log_probability_false = log_joint_probability_of_truth(
		    T, proof_prior, proof_axioms, negation, 250, 4, 20, T_MAP, proof_MAP);

		if (!Theory::is_empty(T_MAP)) {
			print("Highest probability theory after testing whether query is false:\n", stdout);
			T_MAP.print_axioms(stdout, *debug_terminal_printer);
			free(*negation);
			if (negation->reference_count == 0)
				free(negation);
			free(T_MAP);
		}
		...
		print("Answer:\n", stdout);
		print("  true : ", stdout);
		print(probabilities[0], stdout);
		print('\n', stdout);
		print("  false : ", stdout);
		print(probabilities[1], stdout);
		print('\n', stdout);
		fflush(stdout);
		...
		if (enable_streaming) {
			...
			write_json_stream("query", to_string(axiom_str), "Query result", std::to_string(probabilities[0]));
		}
	} else {
		...
		for (unsigned int i = 0; i < answers.size; i++) {
			print("  ", stdout);
			print(answers.keys[i], stdout);
			print(" : ", stdout);
			print(answers.values[i], stdout);
			print('\n', stdout);
		}
		fflush(stdout);
		...
		if (enable_streaming) {
			...
			write_json_stream("query", to_string(axiom_str), to_string(answer_str), std::to_string(answers.values[0]));
		}
