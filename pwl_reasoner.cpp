...
template<typename Stream>
bool read_terms(array<hol_term*>& terms, Stream& in,
    ...
    while (index < tokens.length) {
        ...
        if (term == NULL) {
            ...
            free_tokens(tokens);
            return false;
        } else if (!expect_token(tokens, index, tptp_token_type::SEMICOLON,
                   "semicolon at end of higher-order term") || !terms.add(term)) {
            free(*term);
            free(term);
            free_tokens(tokens);
            return false;
...
bool check_consistency(Theory& T, const ProofAxioms& proof_axioms,
                      const Collector& collector, unsigned int i, unsigned int j,
                      unsigned int t, const char* phase) {
    ...
    if (!success) {
        ...
        fprintf(stderr, "ERROR: Theory consistency check failed at iteration: i = %u, j = %u, t = %u",
                i, j, t);
...
int main(int argc, const char** argv) {
    ...
    theory<natural_deduction<hol_term, false>,
           polymorphic_canonicalizer<true, false, built_in_predicates>> T(seed_axioms, 1000000000);
    ...
    auto theory_element_prior = make_simple_hol_term_distribution<built_in_predicates>(
            constant_prior, geometric_distribution(0.0001),
            very_light_tail_distribution(-40.0),
            0.0199999, 0.01, 0.0000001, 0.17, 0.1, 0.1, 0.01, 0.57, 0.01, 0.01,
            0.1099999, 0.01, 0.0000001, 0.1999999, 0.26, 0.01, 0.01, 0.0000001,
            0.2, 0.2, 0.999999998, 0.000000001, 0.000000001, 0.3, 0.4, 0.2, 0.4,
            -2000.0);
    ...
    auto proof_prior = make_canonicalized_proof_prior(
            axiom_prior, conjunction_introduction_prior, conjunction_elimination_prior,
            universal_introduction_prior, universal_elimination_prior, term_indices_prior,
            poisson_distribution(20.0), 0.00001);
    ...
    for (unsigned int i = 0; i + 1 < lfs.length; i++) {
        ...
        if (lfs.last()->type != hol_term_type::LAMBDA) {
            ...
            answer_question<false>(answers, lfs.last(), 1000, printer, T,
                                proof_prior, proof_axioms);
